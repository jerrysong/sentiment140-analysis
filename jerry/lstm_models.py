...
def get_model(params):
    if params['embedding_type'] == 'randomly initialized':
        embedding_layer = Embedding(
            params['embedding_max_features'], params['embedding_dim'], input_length=params['input_length'])
    else:
...
def get_pretrained_embedding_layer(
        zip_embedding_path, embedding_name, word_index, embedding_dim, input_length, trainable):
    embeddings_index = get_embeddings_index(zip_embedding_path, embedding_name)
    embedding_matrix = get_embedding_matrix(embeddings_index, word_index, embedding_dim)
...
def get_rnn_layers(params):
    ...
    if rnn_type == LSTM or rnn_type == GRU:
        for _ in range(params['rnn_layer_num'] - 1):
            yield rnn_type(params['rnn_dim'], recurrent_dropout=params['recurrent_dropout'], return_sequences=True)

        yield rnn_type(params['rnn_dim'], recurrent_dropout=params['recurrent_dropout'])
